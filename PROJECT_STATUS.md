...
阶段名称：SillyTavern 版本回退

状态：Phase 6 本地实现、专项测试与完整回归通过，待 Android Termux 实机验证

最后更新：2026-07-18（备份删除入口合并及 Phase 6 版本回退本地实现完成，14/14 隔离回归通过，待 Android Termux 实机验证）
...
- 项目根目录 `VERSION` 作为 STermux 单一可信版本来源，当前为 `v0.0.3`
...
- 备份创建按 data、third-party、元数据和完整性验证展示真实阶段；耗时步骤每 10 秒显示已用时间，完成后显示总耗时和备份大小
- 备份菜单将单删与多删合并为一个“删除备份”入口，继续复用空格/逗号、非法过滤、去重、最终清单和默认 N
- SillyTavern 更新中心新增独立“版本回退”入口，普通列表只显示经过验证的最近 5 个正式语义版本
- 回退版本来自官方 remote 的 tag；网络失败时使用本地有效 tag，不接受带 v、预发布、版本不匹配或非当前历史 tag
- 版本回退拒绝 detached HEAD、无 upstream、本地领先/分叉、合并冲突及 tracked 程序文件修改
- 用户确认后必须先创建 protective 完整备份，备份失败不会执行 Git 版本切换
- 在原分支上安全指向目标历史 Commit，保持分支名与 upstream，不进入 detached HEAD
- 回退后使用官方生产依赖参数同步 Node Modules；依赖或验证失败时不报告成功并尝试恢复原 Commit
- 回退日志保存到 `data/state/rollback-history.log`，记录版本、Commit、备份、Git、依赖与最终结果
- 隔离测试已验证回退后仍可由现有更新中心重新升级到较新 release 版本

---

## 当前开发中功能

Phase 6 版本回退已完成本地实现和隔离自动测试，等待 Android Termux 实机验证。Phase 4、独立终端 UI 与早期公开测试补充仍保持各自原有的待实机验证状态。
...
- Phase 6 官方 tag 列表、保护备份、真实 npm 依赖同步、回退后启动与重新升级的 Android Termux 验证
...
- Bash 语法、卸载专项、扩展删除专项、自动备份上限专项与完整隔离回归通过；当前完整测试为 14/14。
...
| 创建进度与耗时心跳 | data、third-party、元数据、完整性阶段及慢任务心跳通过 | Git Bash 通过 | 待测试 | 本地通过 |
| 完整回归 | 14/14 通过 | Git Bash 通过 | 待测试 | 本地通过 |
...
| 功能 | 本地测试 | Linux / 模拟 Termux | Termux 实机 | 状态 |
| ---- | -------- | ------------------- | ----------- | ---- |
| 默认关闭与旧配置安全默认值 | 开关 false、频率 7 天通过 | Git Bash 通过 | 用户确认通过 | 已完成 |
| 开启、关闭与配置持久化 | 临时 user.conf 通过 | Git Bash 通过 | 用户确认通过 | 已完成 |
| 频率 1–30 天与无效输入 | 3/7 天、空值、文本、0、31、负数通过 | Git Bash 通过 | 用户确认通过 | 已完成 |
| 未到时间不创建备份 | 固定 epoch 通过 | Git Bash 通过 | 用户确认通过 | 已完成 |
| 正常到期 scheduled | 真实 Phase 4 备份引擎与元数据通过 | Git Bash 通过 | 用户确认通过 | 已完成 |
| 逾期 catchup 与多周期单份限制 | 错过 4 个周期只新增 1 份通过 | Git Bash 通过 | 用户确认通过 | 已完成 |
| 成功后下一次时间计算 | 从成功 epoch + 当前频率计算通过 | Git Bash 通过 | 用户确认通过 | 已完成 |
| 失败不更新状态并继续重试 | config.yaml 缺失失败、恢复后重试通过 | Git Bash 通过 | 用户确认通过 | 已完成 |
| 同周期去重与并发启动锁 | 重复检查及活动 PID 锁通过 | Git Bash 通过 | 用户确认通过 | 已完成 |
| 备份成功但状态提交中断 | 识别同到期 epoch 元数据，只补写状态不重复备份 | Git Bash 通过 | 用户确认通过 | 已完成 |
| 状态缺失与损坏恢复 | 只重建未来计划、不创建历史备份 | Git Bash 通过 | 用户确认通过 | 已完成 |
| 无效 SillyTavern 路径 | 安全跳过且状态不变 | Git Bash 通过 | 用户确认通过 | 已完成 |
| 关闭期间与重新开启 | 不触发；重新开启不补历史备份 | Git Bash 通过 | 用户确认通过 | 已完成 |
| 统一自动池与 manual 隔离 | protective/scheduled/catchup 共池，manual 保留 | Git Bash 通过 | 用户确认通过 | 已完成 |
| 自定义最大数量 | 现有 1–20 配置及即时轮换回归通过 | Git Bash 通过 | 用户确认通过 | 已完成 |
| 首页状态与自动备份设置页 | 开启/关闭、频率、下次时间和数量通过 | Git Bash 通过 | 用户确认通过 | 已完成 |
| 版本单一来源 | VERSION、首页、设置和自更新读取 v0.0.2 | Git Bash 通过 | 用户确认通过 | 已完成 |
| Bash 语法与完整回归 | 14/14 通过 | Git Bash 通过 | 用户确认通过 | 已完成 |

2026-07-18：用户确认 Phase 5 与大数据备份进度展示 Android Termux 验证无异常，Phase 5 验收完成。
...
| 功能 | 本地测试 | Linux / 模拟 Termux | Termux 实机 | 状态 |
| ---- | -------- | ------------------- | ----------- | ---- |
| 正式 tag 倒序与最近 5 个上限 | 6 个有效旧版本中只保留 1.17.0 至 1.13.0 | Git Bash 通过 | 待测试 | 本地通过 |
| 当前、带 v、预发布、无效与版本不匹配 tag 过滤 | 通过 | Git Bash 通过 | 待测试 | 本地通过 |
| 官方 tag 刷新失败的本地降级 | Mock 网络失败、本地 tag 可用 | Git Bash 通过 | 待测试 | 本地通过 |
| 取消、非法编号与伪造目标 | 均未调用备份或 Git | Git Bash 通过 | 待测试 | 本地通过 |
| tracked 修改与 detached HEAD | 明确拒绝通过 | Git Bash 通过 | 待测试 | 本地通过 |
| upstream、领先与分叉安全条件 | 公共 Git 状态与回退前置检查通过 | Git Bash 通过 | 待测试 | 本地通过 |
| protective 失败阻断 | HEAD 保持不变、Git 未执行 | Git Bash 通过 | 待测试 | 本地通过 |
| protective 成功与正常回退 | 备份后切换到 1.17.0 通过 | Git Bash 通过 | 待测试 | 本地通过 |
| package.json、分支与 upstream 验证 | 版本正确、release/origin-release 保持 | Git Bash 通过 | 待测试 | 本地通过 |
| 用户数据不被 Git 回退删除 | data/chat 与 third-party 哨兵保留 | Git Bash 通过 | 待测试 | 本地通过 |
| 依赖失败不伪报成功 | Mock 失败并恢复原 Commit 通过 | Git Bash 通过 | 待测试 | 本地通过 |
| Git 操作失败 | Mock reset 失败、HEAD 不变 | Git Bash 通过 | 待测试 | 本地通过 |
| 回退后重新升级 | update_available → 1.18.0 成功 | Git Bash 通过 | 待测试 | 本地通过 |
| 回退日志 | 原/目标版本与 Commit、备份/Git/依赖结果通过 | Git Bash 通过 | 待测试 | 本地通过 |
| 第三方代理隔离 | 回退实现无代理配置，非官方 remote 不联网 | Git Bash 通过 | 待测试 | 本地通过 |
| VERSION 单一来源 | VERSION、首页、设置和自更新读取 v0.0.3 | Git Bash 通过 | 待测试 | 本地通过 |
| Bash 语法与完整回归 | 14/14 通过 | Git Bash 通过 | 待测试 | 本地通过 |

Phase 6 未经过 Android Termux 实机验证，因此不得标记为最终完成。
...
完成 Phase 6 版本回退的 Android Termux 验收；实机通过前不进入 Phase 7，也不提前展开其他后续范围。
...
2026-07-18：备份创建增加真实阶段与耗时心跳展示。data、third-party 和完整性校验超过 10 秒时会持续提示已用时间，完成后显示总耗时与备份大小；不伪造百分比、不隐藏 tar 错误，也不改变临时目录、验证、轮换或备份格式。Bash 语法、备份专项及完整 13/13 隔离回归通过，等待 Android Termux 大数据备份与 Ctrl+C 取消验证。

2026-07-18：用户确认 Phase 5 与大数据备份进度展示 Android Termux 验证无异常，Phase 5 更新为已完成，并正式进入 Phase 6。

2026-07-18：备份删除菜单合并为统一单选/多选入口；Phase 6 正式版本 tag 列表、安全回退、protective 备份、依赖同步、失败恢复、日志与回退后重新升级链路完成本地实现。VERSION 更新为 `v0.0.3`，Bash 语法、任务一专项、Phase 6 专项及完整 14/14 隔离回归通过，等待 Android Termux 实机验收。

2026-07-18：Phase 6 回退页面仅保留经过完整验证的最近 5 个正式版本作为展示与可选目标；本地旧 tag 和 Git 历史保持不变。专项测试与完整 14/14 隔离回归通过。
