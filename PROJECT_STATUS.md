...
最后更新：2026-07-17（早期公开测试前核心体验补全本地通过；Phase 4 状态保持待 Android Termux 实机验证）
...
- 第三方扩展管理支持从当前扫描清单单选或多选删除 Git / 非 Git 扩展；成功后同步移除 manual 策略
...
早期公开测试前核心体验补全与独立终端 UI 优化均已完成本地实现和隔离自动测试，等待 Android Termux 实机验证。Phase 4 同时保持待实机验证。
...
- 第三方扩展删除的 Android Termux Git / 非 Git、多选、取消及策略同步验证
...
- 扩展删除复用现有扫描清单与多选解析；Git / 非 Git 均可删除，单项失败不阻断后续项，成功后清理对应 manual 策略并写入现有扩展日志。
...
- Bash 语法、卸载专项、扩展删除专项、自动备份上限专项与完整隔离回归通过；当前完整测试为 12/12。
...
| 完整回归 | 12/12 通过 | Git Bash 通过 | 待测试 | 本地通过 |
...
| 扩展策略同步与日志 | manual 清理、成功/失败日志通过 | Git Bash 通过 | 待测试 | 本地通过 |
...
| 完整隔离边界 | 临时 HOME/STermux/ST/备份/扩展/.bashrc/策略 | Git Bash 通过 | 不适用 | 本地通过 |
| Bash 语法与完整回归 | 12/12 通过 | Git Bash 通过 | 待测试 | 本地通过 |

---

## 下一步

完成早期公开测试前补全、Phase 4 备份、终端 UI 与 Bash 自动进入子功能的 Android Termux 验收；不进入 Phase 5，也不提前展开 Phase 8 其他范围。
