...
阶段名称：SillyTavern 安装与首次配置

状态：Phase 2.5 实机问题回修完成，本地测试通过，待纯净 Termux 再次验收

最后更新：2026-07-16（修复依赖二次确认隐藏及 clone 300 秒硬超时；本地自动测试 7/7 通过）
...
| Phase 2.5 | SillyTavern 安装与首次配置 | 回修完成，待再次实机验收 |
| Phase 3 | 第三方扩展更新   | 未开始 |
| Phase 4 | 备份核心         | 未开始 |
| Phase 5 | 定时备份         | 未开始 |
| Phase 6 | 版本回退         | 未开始 |
...
暂无。两个 Phase 2.5 实机问题已完成回修与本地回归，等待纯净 Termux 再次验收。
...
- Phase 2.5 在未安装 SillyTavern 的纯净 Termux 上完成一次真实从零安装
- Phase 2.5 缺失依赖安装、首次启动及安装日志的 Android Termux 实机验证

---

## 已知问题

暂无已知未修复问题。依赖二次确认与 clone 300 秒硬超时已完成本地修复，等待实机确认。
...
| 功能 | 本地测试 | Linux / 模拟 Termux | Termux 实机 | 状态 |
| ---- | -------- | ------------------- | ----------- | ---- |
| Bash 语法 | 通过 | 未执行（当前无 WSL Linux 发行版） | 待测试 | 待实机验收 |
| 无安装/已有安装菜单分流 | 集成测试通过 | 未执行 | 待测试 | 本地测试通过 |
| 依赖可用、缺失、损坏三态 | Mock 测试通过 | 模拟 Termux 通过 | 待测试 | 本地测试通过 |
| `pkg` 一次确认、实时输出与复验 | Mock `pkg`、`-y` 和输出透传通过 | 模拟 Termux 通过 | 待复测 | 回修完成，待实机 |
| 目标目录五类状态 | 临时目录测试通过 | 未执行 | 待测试 | 本地测试通过 |
| 普通目录拒绝覆盖与危险路径拒绝 | 通过 | 未执行 | 待测试 | 本地测试通过 |
| 不完整安装保留重装 | 通过 | 未执行 | 待测试 | 本地测试通过 |
| 官方 release 安装流程 | 本地 Git 仓库通过，无固定总超时 | 未执行 | 待复测 | 待真实网络验收 |
| 安装成功保存 `ST_PATH` | 通过 | 未执行 | 待测试 | 本地测试通过 |
| 下载失败不保存无效路径 | 通过 | 未执行 | 待测试 | 本地测试通过 |
| 路径保存失败诊断 | 通过 | 未执行 | 待测试 | 本地测试通过 |
| 首次启动复用统一启动逻辑 | 代码路径与 Fixture 启动通过 | 未执行 | 待测试 | 待实机验收 |

---

## 下一步

在未安装 SillyTavern 的纯净 Termux 测试手机上重新执行 Phase 2.5 实机验收：重点确认依赖只需一次确认、pkg/apt 与 clone 进度实时可见、clone 不再于 300 秒终止，并继续验证 `ST_PATH` 保存和首次启动。验收通过前不进入 Phase 3。
