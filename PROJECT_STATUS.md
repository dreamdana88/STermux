...
状态：Phase 2.5 本地测试通过，待 Termux 纯净设备从零安装验收

最后更新：2026-07-16（Phase 2 实机验收通过；Phase 2.5 本地自动测试 7/7 通过）
...
| Phase 2.5 | SillyTavern 安装与首次配置 | 本地通过，待实机验收 |
| Phase 3 | 第三方扩展更新   | 未开始 |
| Phase 4 | 备份核心         | 未开始 |
| Phase 5 | 定时备份         | 未开始 |
| Phase 6 | 版本回退         | 未开始 |
...
- 安装日志、网络超时、安装失败隔离及首次启动复用统一启动逻辑

---

## 当前开发中功能

暂无。Phase 2.5 代码与本地自动测试已完成，等待 Termux 纯净设备实机验收。
...
| `pkg` 缺失依赖安装与复验 | Mock `pkg` 通过 | 模拟 Termux 通过 | 待测试 | 本地测试通过 |
| 目标目录五类状态 | 临时目录测试通过 | 未执行 | 待测试 | 本地测试通过 |
| 普通目录拒绝覆盖与危险路径拒绝 | 通过 | 未执行 | 待测试 | 本地测试通过 |
| 不完整安装保留重装 | 通过 | 未执行 | 待测试 | 本地测试通过 |
| 官方 release 安装流程 | 本地 Git 仓库通过 | 未执行 | 待测试 | 待真实网络验收 |
...
在未安装 SillyTavern 的纯净 Termux 测试手机上执行 Phase 2.5 实机验收：验证依赖检测、真实 release 分支安装、`ST_PATH` 保存和首次启动。验收通过前不进入 Phase 3。
